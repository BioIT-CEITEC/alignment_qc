...
paired_end <- if(args[3]=='pair') TRUE else FALSE
...
if(sum(dm$allCountsMulti) > 1){
  ...
  pdf(paste0(output_folder,"/",input_bam_basename, "_duprateExpDens.pdf"))
  ...
  cat(
    ...
    file=paste0(output_folder,"/",input_bam_basename, "_intercept_slope.txt"), append=FALSE
  )
  ...
  write.table(
    ...
    file=paste0(output_folder,"/",input_bam_basename, "_duprateExpDensCurve.txt"),
  ...
  sink(paste0(output_folder,"/",input_bam_basename, "_multimapPerGene.txt"))
  ...
  pdf(paste0(output_folder,"/",input_bam_basename, "_multimapPerGene.pdf"))
  hist(dm$mhRate, 
       breaks=50, 
       col="red", 
  ...
  pdf(paste0(output_folder,"/",input_bam_basename, "_duprateExpBoxplot.pdf"))
  ...
  pdf(paste0(output_folder,"/",input_bam_basename, "_expressionHist.pdf"))
  ...
  pdf(paste0(output_folder,"/",input_bam_basename, "_readDist.pdf"))
  ...
  pdf(paste0(output_folder,"/",input_bam_basename, "_duprateExpDens.pdf"))
  plot.new()
  text(0.5,0.5,"no counts in the sample")
  dev.off()
  
  system(paste0("touch ",output_folder,"/",input_bam_basename, "_duprateExpDensCurve.txt"))

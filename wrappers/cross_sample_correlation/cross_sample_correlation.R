  ...
  if(length(vcf_files) > 1 & all(file.size(vcf_files) > 0)){
    ...
    if(!all(sapply(all_snps,is.null))){
      ...
      if (nrow(all_snps) > 0){
        ...
        test_overlap_matrix[is.na(test_overlap_matrix)] <- MIN_VALUES_TO_CORELATE + 1
...
# vcf_files <- list.files(path = "/mnt/ssd/ssd_1/snakemake/library2477_210514_MDS23/map_qc/cross_sample_correlation/",pattern = ".snp.vcf",full.names = T)

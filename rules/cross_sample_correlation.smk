...
rule cross_sample_correlation:
    input:  vcfs = expand("qc_reports/cross_sample_correlation/{sample}.snp.vcf",sample = sample_tab.sample_name)
    output: Rdata_for_viz =  "qc_reports/cross_sample_correlation/cross_sample_correlation.Rdata",
...
rule snp_vaf_compute:
    ...
    output: vcf = "qc_reports/cross_sample_correlation/{sample}.snp.vcf",
    log:    "logs/cross_sample_correlation/{sample}_snp_vaf_compute.log"
    params: intersect_bed = "qc_reports/cross_sample_correlation/{sample}.snp_tmp_intersect.bed"
    threads: 1

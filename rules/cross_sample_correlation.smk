...
rule cross_sample_correlation_viz:
    ...
    params: output = BR.remote("qc_reports/all_samples/cross_sample_correlation/cross_sample_correlation")
    threads: 1
...
rule snp_vaf_compute:
    input:  bam = BR.remote("mapped/{sample}.bam"),
            ...
            snp_bed = BR.remote(expand("{ref_dir}/other/snp/{ref}.snp.bed",ref_dir=reference_directory,ref = config["reference"])),
    ...
    params: intersect_bed = BR.remote("qc_reports/all_samples/cross_sample_correlation/{sample}.snp_tmp_intersect.bed")
    threads: 1

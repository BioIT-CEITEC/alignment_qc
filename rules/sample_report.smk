...
def multiqc_report_input(wildcards):
    ...
    if wildcards.sample != "all_samples":
        if config["is_paired"]:
            ...
            input['raw_fastq_R1_report'] = BR.remote("qc_reports/{sample}/raw_fastqc/R1_fastqc.zip")
            input['raw_fastq_R2_report'] = BR.remote("qc_reports/{sample}/raw_fastqc/R2_fastqc.zip")
            ...
            input['raw_fastq_SE_report'] = BR.remote("qc_reports/{sample}/raw_fastqc/SE_fastqc.zip")
        ...
        if config["qc_samtools"]:
            input['qc_samtools'] = BR.remote("qc_reports/{sample}/qc_samtools/{sample}.idxstats.tsv")
        if config["qc_picard_DNA"]:
            #input['qc_picard_DNA'] = "qc_reports/{sample}/qc_picard_DNA/picard.tsv"
            if config["lib_ROI"] != "wgs":
                input['qc_picard_DNA'] = BR.remote(expand("qc_reports/{sample}/qc_picard_DNA/picard_table.per_target.tsv",sample=sample_tab.sample_name))
                input['qc_picard_DNA_metrics'] = BR.remote(expand("qc_reports/{sample}/qc_picard_DNA/picard.per_target.tsv",sample=sample_tab.sample_name))
            else:
                input['qc_picard_DNA'] = BR.remote(expand("qc_reports/{sample}/qc_picard_DNA/picard_table.wgs_chart.tsv",sample=sample_tab.sample_name))
                input['qc_picard_DNA_metrics'] = BR.remote(expand("qc_reports/{sample}/qc_picard_DNA/picard.wgs_chart.pdf",sample=sample_tab.sample_name))
